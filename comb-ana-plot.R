...
#time specifications
df$tspec[df$task==1] <- 90 #time spec for task 1 in seconds
...
#or based on derivation from data, 80th percentile from sat above 4
quantile(df$time[df$sat >= 4 & df$task==3], probs = seq(0, 1, by= 0.1))

df$tspec[df$task==4] <- 180 #time spec for task 4 in seconds
#or based on derivation from data, 80th percentile from sat above 4
quantile(df$time[df$sat >= 4 & df$task==4], probs = seq(0, 1, by= 0.1))
